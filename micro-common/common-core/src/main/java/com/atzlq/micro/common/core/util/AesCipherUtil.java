...
public class AesCipherUtil {
    ...
    public static String enCrypto(String str) {
        try {
            ...
            throw new GlobleException(20021,"getInstance()方法异常:" + e.getMessage());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:{}", e.getMessage());
            throw new GlobleException(20022,"Base64加密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new GlobleException(20023,"初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:{}", e.getMessage());
            throw new GlobleException(20024,"加密异常，密钥有误:" + e.getMessage());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new GlobleException(20025,"Base64解密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new GlobleException(20026,"初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:{}", e.getMessage());
            throw new GlobleException(20027,"解密异常，密钥有误:" + e.getMessage());
        }
    }
}
